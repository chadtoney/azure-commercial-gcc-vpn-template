...
$subId = if ($SubscriptionId) { $SubscriptionId } else { $config['COMMERCIAL_SUBSCRIPTION_ID'] }
$rgName = if ($ResourceGroupName) { $ResourceGroupName } else { $config['COMMERCIAL_RESOURCE_GROUP'] }
$location = $config['COMMERCIAL_REGION']
...
$bicepParams = @{
  'prefix' = $prefix
  'location' = $location
  'tenantId' = $config['COMMERCIAL_TENANT_ID']
  'vnetAddressPrefixes' = @($config['COMMERCIAL_ADDRESS_PREFIX'])
  'gatewaySubnetPrefix' = $config['COMMERCIAL_GATEWAY_SUBNET']
  'workloadSubnetPrefix' = $config['COMMERCIAL_WORKLOAD_SUBNET']
  'vpnGatewaySku' = $config['COMMERCIAL_GATEWAY_SKU']
  'remoteGatewayPublicIp' = $config['COMMERCIAL_REMOTE_GATEWAY_IP']
  'remoteAddressPrefixes' = @($config['COMMERCIAL_REMOTE_ADDRESS_PREFIX'])
  'sharedKey' = $plainSharedKey
  'createKeyVault' = [bool]::Parse($config['CREATE_KEY_VAULT'])
}

Write-Host 'Setting cloud to AzureCloud...' -ForegroundColor Cyan
...
Write-Host 'Deploying commercial VPN infrastructure...' -ForegroundColor Green
$deploymentName = "commercial-vpn-$(Get-Date -Format yyyyMMddHHmmss)"
...
az deployment group create `
  ...
  --template-file "$PSScriptRoot/../infra/main-commercial.bicep" `
  @params | Out-Null

Write-Host 'Commercial VPN deployment completed successfully.' -ForegroundColor Green
